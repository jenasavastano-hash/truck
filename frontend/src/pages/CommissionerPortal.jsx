...
import api from '../api';
import { ShieldAlert, MessageSquare, Radio, LogOut, ListOrdered, ClipboardList, Send, Check, Navigation, Bell, Clock, Phone, Wallet, User } from 'lucide-react';
...
export default function CommissionerPortal() {
  ...
  const loadAll = () => {
    Promise.all([
      api.get('/commissioner/online'),
      api.get('/commissioner/balance').catch(() => ({ data: { balance: 0 } })),
      api.get('/commissioner/requests'),
      api.get('/commissioner/orders'),
      api.get('/commissioner/notifications').catch(() => ({ data: [] }))
    ])
  ...
  useEffect(() => {
    ...
    async function checkPayment() {
      try {
        await api.get(`/commissioner/payment/${paymentId}/status`);
      } catch (err) {
  ...
  const markNotificationRead = (id, closeMenu = false) => {
    api.patch(`/commissioner/notifications/${id}/read`).then(() => {
  ...
  const handleNotificationClick = (n) => {
    ...
    if (n.type === 'commissioner_new_request' || n.type === 'commissioner_confirmed') {
      setTab(n.type === 'commissioner_confirmed' ? 'orders' : 'requests');
    }
  };

  const toggleOnline = () => {
    api
      .patch('/commissioner/online', { isOnline: !online })
  ...
  const handleRespond = (requestId) => {
    ...
    api
      .post(`/commissioner/requests/${requestId}/respond`, { etaMinutes: eta, price })
      .then(() => {
  ...
  const setOrderStatus = (requestId, status) => {
    ...
    api
      .patch(`/commissioner/orders/${requestId}/status`, { status })
      .then(loadAll)
  ...
  const goTopup = () => navigate('/commissioner/balance-topup');
              ...
              orders.map((o) => (
                ...
                >
                  <EvacCard
                    ...
                    status={o.status}
                    subtitle={[
                      `${o.authorName || 'Клиент'} · ${o.authorPhone || '—'} · ${o.price} ₽`,
