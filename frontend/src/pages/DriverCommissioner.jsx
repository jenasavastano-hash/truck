...
import api from '../api';
import { ShieldAlert, ArrowLeft, MapPin, MessageSquare, Send, Check, Clock, Navigation, Phone, X, AlertTriangle } from 'lucide-react';
import { formatDateMsk } from '../utils/dateFormatter';
import EvacCard from '../components/evacuator/EvacCard';

const DRAFT_KEY = 'driver_commissioner_draft_v1';
...
export default function DriverCommissioner() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get('/driver/commissioner/requests')
    ])
  ...
  const handleCreate = (e) => {
    ...
    setCreating(true);
    api
      .post('/driver/commissioner/requests', {
      ...
      .then(() => {
        ...
        return api.get('/driver/commissioner/requests');
      })
  ...
  const handleConfirm = (requestId, responseId) => {
    ...
    setError('');
    api
      .post(`/driver/commissioner/requests/${requestId}/confirm`, { responseId })
      .then(() => api.get('/driver/commissioner/requests'))
      .then((res) => setRequests(Array.isArray(res.data) ? res.data : []))
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-orange-50 to-slate-50 p-4 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm text-center">
          <p className="text-slate-600 mb-4">Вызов аварийного комиссара для вашего парка отключён.</p>
          <button
            type="button"
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
          ...
          >
            {activeRequest.responses && activeRequest.responses.length > 0 && (activeRequest.status === 'created' || activeRequest.status === 'has_responses') && (
              <div className="mt-4 space-y-2">
                <p className="text-sm font-semibold text-slate-800">Отклики комиссаров</p>
                  ...
                  .map((r) => (
                    ...
                    >
                      <div className="min-w-0">
                        <p className="font-semibold text-slate-900 truncate">{r.commissionerName || 'Комиссар'}</p>
                        <p className="text-sm text-slate-600">
                          ETA: <strong>{r.etaMinutes} мин</strong> · Цена: <strong>{r.price} ₽</strong>
                        </p>
                        {r.commissionerPhone && <p className="text-xs text-slate-500 mt-0.5">Тел: {r.commissionerPhone}</p>}
                      </div>
                      <button
                        type="button"
        ...
        <div className="rounded-2xl bg-white border border-orange-100 p-4 shadow-sm">
          <p className="text-slate-600 text-sm mb-4">
            Сейчас на линии: <strong>{settings?.commissionersOnlineCount ?? 0}</strong> комиссаров
          ...
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <label className="block">
              ...
              <textarea
                ...
                placeholder="Что случилось, детали ДТП..."
                rows={2}
                className="mt-1 w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
              />
            </label>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                title="Добавить координаты (помогает комиссару быстрее приехать)"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={() => setConfirmModal(null)}>
          ...
          >
            <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
              <p className="font-bold text-slate-900">Подтвердить комиссара</p>
            ...
            <div className="p-4 space-y-3 text-sm">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="font-semibold text-slate-900">{confirmModal.response?.commissionerName || 'Комиссар'}</p>
                ...
                {confirmModal.response?.commissionerPhone && (
                  <a className="mt-2 inline-flex items-center gap-2 text-indigo-600 font-semibold" href={`tel:${confirmModal.response.commissionerPhone}`}>
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-amber-900">
                ...
                <p className="mt-1">
                  ...
                  {settings?.requestCreationPrice > 0 ? ` (${settings.requestCreationPrice} ₽)` : ''}. Оплата услуги комиссара — <strong>на месте</strong>.
                </p>
              </div>
