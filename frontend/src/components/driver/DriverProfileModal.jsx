...
import { User, Shield, Eye, EyeOff, CheckCircle2, XCircle, Phone, KeyRound, CreditCard, Car, FileCheck, Building2, UserCircle, Lock } from 'lucide-react';
import api from '../../api';
...
export default function DriverProfileModal({ driver, isOpen, onClose }) {
  const [activeTab, setActiveTab] = useState('profile');
  const [showCurrentPassword, setShowCurrentPassword] = useState(false);
  const [showNewPassword, setShowNewPassword] = useState(false);
  const [newUsername, setNewUsername] = useState(driver?.username || '');
  ...
  const handleSaveSecurity = async (e) => {
    ...
    try {
      ...
      try {
        const loginUsername = newUsername || driver?.username;
  ...
  const profileFields = [
    ...
    { 
      label: 'Телефон', 
      value: driver?.phone || driver?.username || '—', 
      icon: Phone,
    ...
    { 
      ...
      value: driver?.licenseSerial || driver?.licenseNumber || driver?.licenseDate
        ? [driver.licenseSerial, driver.licenseNumber, driver.licenseDate].filter(Boolean).join(', ')
    ...
    { 
      ...
      value: driver?.regNumber 
        ? [driver.brand, driver.model].filter(Boolean).join(' ') || driver.regNumber 
        : '—',
      subValue: driver?.regNumber ? `(${driver.regNumber})` : null,
      icon: Car,
      gradient: 'from-orange-500 to-orange-600'
    },
    { 
      label: 'Верификация', 
      value: driver?.isVerified ? 'Подтверждён' : 'Ожидает',
      icon: CheckCircle2,
      gradient: driver?.isVerified ? 'from-emerald-500 to-emerald-600' : 'from-amber-500 to-amber-600',
      badge: true
    }
  ];
  ...
  return (
    <>
      <Modal
        ...
        title=""
        size="lg"
        className="max-h-[95vh] sm:max-h-[90vh] overflow-hidden"
      >
            ...
            {tabs.map((tab) => {
              ...
              return (
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-4 sm:px-6 py-3 rounded-xl font-semibold transition-all relative ${
                    isActive
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className={`font-bold text-slate-800 text-sm sm:text-base ${field.badge ? (driver?.isVerified ? 'text-emerald-600' : 'text-amber-600') : ''}`}>
                            {field.value}
                          </p>
                          {field.subValue && (
                            <p className="text-xs text-slate-600 mt-0.5">{field.subValue}</p>
                          )}
                        </div>
                      </div>
                    </motion.div>
